...
{
    public partial class AYıkama5 : Form
    {
        public AYıkama5()
        {
            InitializeComponent();
        ...
        private void AYıkama5_Load(object sender, EventArgs e)
        {
            ...
            while (Sdr.Read())
            {
                ...
                {
                    alınanMakine5.Items.Add(Sdr.GetString(i));
                }
            }
            Sdr.Close();
            con.Close();
            string alınmasaati = alınanSaat5.Text;
            String alımtarihi = alınmaTarihi5.Text;
            ...
            A5dataGridView.DataSource = DS2.Tables[0];
            A5dataGridView.AllowUserToAddRows = false;
            con.Close();
        }
        int count;
        int count2;

        private void islemButonu5_Click(object sender, EventArgs e)
        {
            if (secimTC5.Text != "")
            {
                String secimtc = secimTC5.Text;
                String alımtarihi = alınmaTarihi5.Text;
                SqlConnection con = new SqlConnection();
                con.ConnectionString = "Data Source=DESKTOP-SN77AA1;Initial Catalog=kuru.temizleme;Integrated Security=True";
                SqlCommand cmd = new SqlCommand();
                ...
                if (DS.Tables[0].Rows.Count != 0)
                {
                    ...
                    secimAdSoyad5.Text = DS.Tables[0].Rows[0][4].ToString();
                    /* if (DS3.Tables[0].Rows.Count == 0)*/
                    if (count2 <= 0)
                    {
                        string alınmasaati = alınanSaat5.Text;
                        string alınanmakine = alınanMakine5.Text;
                        String alınmatarihi = alınmaTarihi5.Text;
                        ...
                        if (alınanMakine5.SelectedIndex != -1 && count <= 0)
                        {
                            String tc2 = secimTC5.Text;
                            String adsoyad2 = secimAdSoyad5.Text;
