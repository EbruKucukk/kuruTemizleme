...
{
    public partial class AYıkama92 : Form
    {
        public AYıkama92()
        {
            InitializeComponent();
        ...
        private void AYıkama92_Load(object sender, EventArgs e)
        {
            ...
            while (Sdr.Read())
            {
                ...
                {
                    alınanMakine12.Items.Add(Sdr.GetString(i));
                }
            }
            Sdr.Close();
            con.Close();
            string alınmasaati = alınanSaat12.Text;
            String alımtarihi = alınmaTarihi12.Text;
            ...
            A12dataGridView.DataSource = DS2.Tables[0];
            A12dataGridView.AllowUserToAddRows = false;
            con.Close();
        }
        int count;
        int count2;

        private void islemButonu12_Click(object sender, EventArgs e)
        {
            if (secimTC12.Text != "")
            {
                String secimtc = secimTC12.Text;
                String alımtarihi = alınmaTarihi12.Text;
                SqlConnection con = new SqlConnection();
                con.ConnectionString = "Data Source=DESKTOP-SN77AA1;Initial Catalog=kuru.temizleme;Integrated Security=True";
                SqlCommand cmd = new SqlCommand();
                ...
                if (DS.Tables[0].Rows.Count != 0)
                {
                    ...
                    secimAdSoyad12.Text = DS.Tables[0].Rows[0][4].ToString();
                    if (count2 <= 0)
                    {
                        string alınmasaati = alınanSaat12.Text;
                        string alınanmakine = alınanMakine12.Text;
                        String alınmatarihi = alınmaTarihi12.Text;
                        ...
                        if (alınanMakine12.SelectedIndex != -1 && count <= 0)
                        {
                            String tc2 = secimTC12.Text;
                            String adsoyad2 = secimAdSoyad12.Text;
