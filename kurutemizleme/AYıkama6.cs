...
{
    public partial class AYıkama6 : Form
    {
        public AYıkama6()
        {
            InitializeComponent();
        ...
        private void AYıkama6_Load(object sender, EventArgs e)
        {
            ...
            while (Sdr.Read())
            {
                ...
                {
                    alınanMakine6.Items.Add(Sdr.GetString(i));
                }
            }
            Sdr.Close();
            con.Close();
            string alınmasaati = alınanSaat6.Text;
            String alımtarihi = alınmaTarihi6.Text;
            ...
            A6dataGridView.DataSource = DS2.Tables[0];
            A6dataGridView.AllowUserToAddRows = false;
            con.Close();
        }
        int count;
        int count2;

        private void islemButonu6_Click(object sender, EventArgs e)
        {
            if (secimTC6.Text != "")
            {
                String secimtc = secimTC6.Text;
                String alımtarihi = alınmaTarihi6.Text;
                SqlConnection con = new SqlConnection();
                con.ConnectionString = "Data Source=DESKTOP-SN77AA1;Initial Catalog=kuru.temizleme;Integrated Security=True";
                SqlCommand cmd = new SqlCommand();
                ...
                if (DS.Tables[0].Rows.Count != 0)
                {
                    ...
                    secimAdSoyad6.Text = DS.Tables[0].Rows[0][4].ToString();
                    if (count2 <= 0)
                    {
                        string alınmasaati = alınanSaat6.Text;
                        string alınanmakine = alınanMakine6.Text;
                        String alınmatarihi = alınmaTarihi6.Text;
                        ...
                        if (alınanMakine6.SelectedIndex != -1 && count <= 0)
                        {
                            String tc2 = secimTC6.Text;
                            String adsoyad2 = secimAdSoyad6.Text;
