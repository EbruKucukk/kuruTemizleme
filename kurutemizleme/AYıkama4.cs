...
{
    public partial class AYıkama4 : Form
    {
        public AYıkama4()
        {
            InitializeComponent();
        ...
        private void AYıkama4_Load(object sender, EventArgs e)
        {
            ...
            while (Sdr.Read())
            {
                ...
                {
                    alınanMakine4.Items.Add(Sdr.GetString(i));
                }
            }
            Sdr.Close();
            con.Close();
            string alınmasaati = alınanSaat3.Text;
            String alımtarihi = alınmaTarihi4.Text;
            ...
            A4dataGridView.DataSource = DS2.Tables[0];
            A4dataGridView.AllowUserToAddRows = false;
            con.Close();
        }
        int count;
        int count2;

        private void islemButonu3_Click(object sender, EventArgs e)
        {
            if (secimTC4.Text != "")
            {
                String secimtc = secimTC4.Text;
                String alımtarihi = alınmaTarihi4.Text;
                SqlConnection con = new SqlConnection();
                con.ConnectionString = "Data Source=DESKTOP-SN77AA1;Initial Catalog=kuru.temizleme;Integrated Security=True";
                SqlCommand cmd = new SqlCommand();
                ...
                if (DS.Tables[0].Rows.Count != 0)
                {
                    ...
                    secimAdSoyad4.Text = DS.Tables[0].Rows[0][4].ToString();
                    /* if (DS3.Tables[0].Rows.Count == 0)*/
                    if (count2 <= 0)
                    {
                        string alınmasaati = alınanSaat3.Text;
                        string alınanmakine = alınanMakine4.Text;
                        String alınmatarihi = alınmaTarihi4.Text;
                        ...
                        if (alınanMakine4.SelectedIndex != -1 && count <= 0)
                        {
                            String tc2 = secimTC4.Text;
                            String adsoyad2 = secimAdSoyad4.Text;
