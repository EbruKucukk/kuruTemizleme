...
{
    public partial class AYıkama91 : Form
    {
        public AYıkama91()
        {
            InitializeComponent();
        ...
        private void AYıkama91_Load(object sender, EventArgs e)
        {
            ...
            while (Sdr.Read())
            {
                ...
                {
                    alınanMakine11.Items.Add(Sdr.GetString(i));
                }
            }
            Sdr.Close();
            con.Close();
            string alınmasaati = alınanSaat11.Text;
            String alımtarihi = alınmaTarihi11.Text;
            ...
            A11dataGridView.DataSource = DS2.Tables[0];
            A11dataGridView.AllowUserToAddRows = false;
            con.Close();
        }
        int count;
        int count2;

        private void islemButonu11_Click(object sender, EventArgs e)
        {
            if (secimTC11.Text != "")
            {
                String secimtc = secimTC11.Text;
                String alımtarihi = alınmaTarihi11.Text;
                SqlConnection con = new SqlConnection();
                con.ConnectionString = "Data Source=DESKTOP-SN77AA1;Initial Catalog=kuru.temizleme;Integrated Security=True";
                SqlCommand cmd = new SqlCommand();
                ...
                if (DS.Tables[0].Rows.Count != 0)
                {
                    ...
                    secimAdSoyad11.Text = DS.Tables[0].Rows[0][4].ToString();
                    if (count2 <= 0)
                    {
                        string alınmasaati = alınanSaat11.Text;
                        string alınanmakine = alınanMakine11.Text;
                        String alınmatarihi = alınmaTarihi11.Text;
                        ...
                        if (alınanMakine11.SelectedIndex != -1 && count <= 0)
                        {
                            String tc2 = secimTC11.Text;
                            String adsoyad2 = secimAdSoyad11.Text;
