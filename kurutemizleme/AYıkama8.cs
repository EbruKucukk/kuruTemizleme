...
{
    public partial class AYıkama8 : Form
    {
        public AYıkama8()
        {
            InitializeComponent();
        ...
        private void AYıkama8_Load(object sender, EventArgs e)
        {
            ...
            while (Sdr.Read())
            {
                ...
                {
                    alınanMakine8.Items.Add(Sdr.GetString(i));
                }
            }
            Sdr.Close();
            con.Close();
            string alınmasaati = alınanSaat8.Text;
            String alımtarihi = alınmaTarihi8.Text;
            ...
            A8dataGridView.DataSource = DS2.Tables[0];
            A8dataGridView.AllowUserToAddRows = false;
            con.Close();
        }
        int count;
        int count2;

        private void islemButonu8_Click(object sender, EventArgs e)
        {
            if (secimTC8.Text != "")
            {
                String secimtc = secimTC8.Text;
                String alımtarihi = alınmaTarihi8.Text;
                SqlConnection con = new SqlConnection();
                con.ConnectionString = "Data Source=DESKTOP-SN77AA1;Initial Catalog=kuru.temizleme;Integrated Security=True";
                SqlCommand cmd = new SqlCommand();
                ...
                if (DS.Tables[0].Rows.Count != 0)
                {
                    ...
                    secimAdSoyad8.Text = DS.Tables[0].Rows[0][4].ToString();
                    if (count2 <= 0)
                    {
                        string alınmasaati = alınanSaat8.Text;
                        string alınanmakine = alınanMakine8.Text;
                        String alınmatarihi = alınmaTarihi8.Text;
                        ...
                        if (alınanMakine8.SelectedIndex != -1 && count <= 0)
                        {
                            String tc2 = secimTC8.Text;
                            String adsoyad2 = secimAdSoyad8.Text;
