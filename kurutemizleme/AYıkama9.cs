...
{
    public partial class AYıkama9 : Form
    {
        public AYıkama9()
        {
            InitializeComponent();
        ...
        private void AYıkama9_Load(object sender, EventArgs e)
        {
            ...
            while (Sdr.Read())
            {
                ...
                {
                    alınanMakine9.Items.Add(Sdr.GetString(i));
                }
            }
            Sdr.Close();
            con.Close();
            string alınmasaati = alınanSaat9.Text;
            String alımtarihi = alınmaTarihi9.Text;
            ...
            A9dataGridView.DataSource = DS2.Tables[0];
            A9dataGridView.AllowUserToAddRows = false;
            con.Close();
        }
        int count;
        int count2;

        private void islemButonu9_Click(object sender, EventArgs e)
        {
            if (secimTC9.Text != "")
            {
                String secimtc = secimTC9.Text;
                String alımtarihi = alınmaTarihi9.Text;
                SqlConnection con = new SqlConnection();
                con.ConnectionString = "Data Source=DESKTOP-SN77AA1;Initial Catalog=kuru.temizleme;Integrated Security=True";
                SqlCommand cmd = new SqlCommand();
                ...
                if (DS.Tables[0].Rows.Count != 0)
                {
                    ...
                    secimAdSoyad9.Text = DS.Tables[0].Rows[0][4].ToString();
                    if (count2 <= 0)
                    {
                        string alınmasaati = alınanSaat9.Text;
                        string alınanmakine = alınanMakine9.Text;
                        String alınmatarihi = alınmaTarihi9.Text;
                        ...
                        if (alınanMakine9.SelectedIndex != -1 && count <= 0)
                        {
                            String tc2 = secimTC9.Text;
                            String adsoyad2 = secimAdSoyad9.Text;
