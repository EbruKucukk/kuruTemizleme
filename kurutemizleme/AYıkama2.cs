...
{
    public partial class AYıkama2 : Form
    {
        public AYıkama2()
        {
            InitializeComponent();
        ...
        private void AYıkama2_Load(object sender, EventArgs e)
        {
            ...
            while (Sdr.Read())
            {
                ...
                {
                    alınanMakine2.Items.Add(Sdr.GetString(i));
                }
            }
            Sdr.Close();
            con.Close();
            string alınmasaati = alınanSaat2.Text;
            String alımtarihi = alınmaTarihi2.Text;
            ...
            A2dataGridView.DataSource = DS2.Tables[0];
            A2dataGridView.AllowUserToAddRows = false;
            con.Close();
        }
        int count;
        int count2;

        private void islemButonu2_Click(object sender, EventArgs e)
        {
            if (secimTC2.Text != "")
            {
                String secimtc = secimTC2.Text;
                String alımtarihi = alınmaTarihi2.Text;
                SqlConnection con = new SqlConnection();
                con.ConnectionString = "Data Source=DESKTOP-SN77AA1;Initial Catalog=kuru.temizleme;Integrated Security=True";
                SqlCommand cmd = new SqlCommand();
                ...
                if (DS.Tables[0].Rows.Count != 0)
                {
                    ...
                    secimAdSoyad2.Text = DS.Tables[0].Rows[0][4].ToString();
                    if (count2 <= 0)
                    {
                        string alınmasaati = alınanSaat2.Text;
                        string alınanmakine = alınanMakine2.Text;
                        String alınmatarihi = alınmaTarihi2.Text;
                        ...
                        if (alınanMakine2.SelectedIndex != -1 && count <= 0)
                        {
                            String tc2 = secimTC2.Text;
                            String adsoyad2 = secimAdSoyad2.Text;
